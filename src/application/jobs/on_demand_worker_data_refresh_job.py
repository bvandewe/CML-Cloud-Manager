...
class OnDemandWorkerDataRefreshJob(ScheduledBackgroundJob):
    ...
    def configure(self, service_provider=None, **kwargs):
        ...
        """
        logger.debug(
            f"🔧 Configuring OnDemandWorkerDataRefreshJob for {self.worker_id}"
        )
        ...
        with tracer.start_as_current_span(
            ...
            try:
                if not self._service_provider:
                    logger.error(
                        f"❌ service_provider not configured for on-demand job {self.worker_id}"
                    )
                    return
                ...
                try:
                    ...
                    if metrics_result.status != 200:
                        logger.warning(
                            f"⚠️ Metrics refresh failed for worker {self.worker_id}: "
                            f"{metrics_result.detail}"
                        )
                        span.set_attribute("metrics.refresh.success", False)
                        span.set_attribute(
                            "metrics.error.message", metrics_result.detail
                        )
                    else:
                        logger.info(
                            f"✅ Metrics refresh completed for worker {self.worker_id}"
                        )
                    ...
                    if worker_running and cml_ready:
                        labs_result = await mediator.execute_async(
                            RefreshWorkerLabsCommand(worker_id=self.worker_id)
                        )

                        if labs_result.status != 200:
                            logger.warning(
                                f"⚠️ Labs refresh failed for worker {self.worker_id}: "
                                f"{labs_result.detail}"
                            )
                            span.set_attribute("labs.refresh.success", False)
                            span.set_attribute("labs.error.message", labs_result.detail)
                        else:
                            logger.info(
                                f"✅ Labs refresh completed for worker {self.worker_id}"
                            )
                    ...
                    if overall_success:
                        ...
                        logger.warning(
                            f"⚠️ On-demand data refresh had errors for worker {self.worker_id}"
                        )
