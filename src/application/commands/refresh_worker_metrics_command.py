    ...
    async def handle_async(
        ...
        try:
            ...
            with tracer.start_as_current_span("sync_ec2_status") as span:
                log.debug(
                    f"Orchestrating EC2 status sync for worker {command.worker_id}"
                )
                ec2_result = await self.mediator.execute_async(
                    SyncWorkerEC2StatusCommand(worker_id=command.worker_id)
                )
                ...
                if ec2_result.status == 200:
                    ...
                    log.warning(
                        f"EC2 status sync failed for worker {command.worker_id}: {ec2_result.detail}"
                    )
            ...
            if results.get("ec2_sync", {}).get("worker_status") == "running":
                with tracer.start_as_current_span("collect_cloudwatch_metrics") as span:
                    log.debug(
                        f"Orchestrating CloudWatch metrics collection for worker {command.worker_id}"
                    )
                ...
                log.debug(
                    f"Skipping CloudWatch metrics for worker {command.worker_id} - not running"
                )
                results["cloudwatch_metrics"] = {
                    "skipped": True,
            ...
            if results.get("ec2_sync", {}).get("worker_status") == "running":
                with tracer.start_as_current_span("sync_cml_data") as span:
                    log.debug(
                        f"Orchestrating CML data sync for worker {command.worker_id}"
                    )
                    ...
                    if cml_result.status == 200:
                        ...
                        log.warning(
                            f"CML data sync failed for worker {command.worker_id}: {cml_result.detail}"
                        )
                ...
                log.debug(
                    f"Skipping CML data sync for worker {command.worker_id} - not running"
                )
                results["cml_sync"] = {
                    "skipped": True,
                    "reason": "Worker not running",
