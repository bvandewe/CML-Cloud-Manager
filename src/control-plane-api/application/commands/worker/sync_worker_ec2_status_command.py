        ...
        try:
            ...
            with tracer.start_as_current_span("query_ec2_instance_status") as span:
                ...
                if instance_details:
                    ...
                    # Recalculate HTTPS endpoint from public IP
                    # Always update endpoint when public_ip changes or endpoint is empty
                    # This handles: 1) Initial population, 2) IP changes after stop/start cycles
                    if instance_details.public_ip:
                        expected_endpoint = f"https://{instance_details.public_ip}"
                        if worker.state.https_endpoint != expected_endpoint:
                            old_endpoint = worker.state.https_endpoint
                            worker.update_endpoint(
                                https_endpoint=expected_endpoint,
                                public_ip=instance_details.public_ip,
                            )
                            log.info(
                                f"Updated HTTPS endpoint for worker {command.worker_id}: "
                                f"{old_endpoint or 'None'} -> {expected_endpoint}"
                            )
                    elif worker.state.https_endpoint:
                        # No public IP but endpoint exists - clear stale endpoint
                        log.info(
                            f"Clearing stale HTTPS endpoint for worker {command.worker_id}: "
                            f"{worker.state.https_endpoint} -> None (no public IP)"
                        )
                        worker.update_endpoint(https_endpoint=None, public_ip=None)
